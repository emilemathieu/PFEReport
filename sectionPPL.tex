...
\begin{equation*}
\gamma(\mbx) := \prod_{i=1}^{|\mbx|}{p_{F_i}(x_i~|~x_{1:i-1})} \prod_{j=1}^{|\mby|}{p_{G_i}(y_j~|~x_{1:\tau(j)})}
\end{equation*}

where Z is the normalizing constant $Z := \int{\gamma(\mbx) d(\mbx)}$ and $\tau$ is a mapping from the index $j$ of the \texttt{observe} statement to the index of the last \texttt{sample} statement encountered before this \texttt{observe} statement during the execution of the program.
...
%Implementing an inference algorithm in \gls{PPL}s amounts to defining an appropriate version of the infer function and checkpoint handlers for \texttt{sample} and \texttt{observe}.
...
Imagine doing a random walk in the space of execution traces of a computation

We have successfully created a random walk in the space of executions. Moreover, it matches the desired binomial distribution. However, we have not handled factor statements (or used the computation scores in any way). This will not match the desired distribution when the computation contains factors (or when the number of random choices may change across executions). The Metropolis Hastings algorithm gives a way to ‘patch up’ this random walk to get the right distribution.
...
f there is a large number of execution paths, clearly, we cannot explore all paths individually. This leaves two possibilities: either we reason about paths more abstractly, or we explore only a subset of paths. In these notes, we focus on the second possibility.
