...
\begin{equation}
p(X_1,\dots,X_n|\alpha,H_0) = \int{\left(\prod_{i=1}^n p(X_i|P) \right) dP(P|\alpha,H_0)}
\end{equation}
...
\textcolor{red}{WHERE TO PUT THIS PART AND IS IT USEFUL ??}
...
Being Bayesian, we need a prior over the mixing distribution $P$. %, and the most common prior to use is a \gls{DP}.
...
\begin{equation*}
\begin{aligned}
\mathbb{P}_{\rho,H_0}(V_0 \in dv_0) &= f_\rho(v_0)dv_0 \\
...
%     \caption{PK hybrid sampler’s graphical model. Each node represents a variable used in the augmented representation for the joint distribution. The nodes with dashed lines represent the algorithmic inputs. Source: \cite{LomeliThesis}s} 
...
\textcolor{red}{Relationship between size-biased and stick breaking parameterisations ??}
