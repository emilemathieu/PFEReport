...
\centering
    ...
    \caption{Histogram of $1000$ samples from the $-\log \text{Beta}(1,2)$ \acrlong{PK} random measure.}
    \label{fig:logBetaPK} 
\end{figure}
...
We use this \gls{DP} construction as a prior over mixture components so as to build an infinite mixture model (see code sample \ref{code:IMM}).

The real code of the size-biased processes and the infinite mixture model which has been used for our experiments can be found on Github \footnote{\url{https://github.com/emilemathieu/Turing.jl/tree/feature-328/src/models}}.
...
By using \gls{SMC}, our random hyperparameters (see code (\ref{code:IMM})) would also be included in the latent space and sampled at the same time as the mixture components and assignments. This is obviously inefficient and \acrlong{PMCMC} methods should perform better for this random parameters setting.
We therefore started by using \acrlong{PG} (see Section \ref{PMCMC}) targeting the cluster components and assignments with the conditional \gls{SMC} and the hyper-parameters with an \gls{HMC}.
...
Unsurprisingly we observe a big path degeneracy issue.
Figure \ref{fig:posterior_samples_time} shows 100 samples from the posterior distributions of mixture components $X_1$ and $X_{82}$. One can note that samples from $X_1$ are very ``sticky'', compared to samples from  $X_{82}$.
Figure \ref{fig:ess} highlights this phenomenon, since we can see that the first $X_t$'s drastically have lower \gls{ESS} than the $X_t$'s at the end of the sequence ($t$ close to 82). 
The higher number of particles is used in the \acrlong{PG} algorithm, the less visible is this issue but the more computationally intensive it is.

This is a well-known issue, which is tackled in the \gls{PGAS} and \gls{IPMCMC} algorithms (see Section \ref{PMCMC} for more details).
\gls{PMMH} should bring symmetry in the \gls{ESS} graph of Figure \ref{fig:ess}, i.e. all $X_t$'s should have in average the same \gls{ESS}. Yet, it will not fix the path degeneracy issue, and posterior samples will still have low \gls{ESS}.
...
We remind that the code of the size-biased processes and the infinite mixture model can be found on Github \footnote{\url{https://github.com/emilemathieu/Turing.jl/tree/feature-328/src/models}}.
